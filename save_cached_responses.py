...
def cache_api_response(my_saved_object):
    ...
    url = u"https://gtr-api.herokuapp.com/search/{}?automated=true".format(entity_term)
    r = requests.get(url)
    print r
    print url
    my_saved_object.api_response = r.json()

    url = u"https://gtr-api.herokuapp.com/search/{}?oa=true&automated=true".format(entity_term)
    r = requests.get(url)
    print r
    print url
